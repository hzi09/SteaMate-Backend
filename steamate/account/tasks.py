...
def fetch_and_save_user_games(user_id):
    ...
    for i in range(len(appids)):
        ...
        user_library_games.append(UserLibraryGame(user=user, game=game, playtime=playtimes[i]))

    try:
        with transaction.atomic():
            if user_library_games:
                UserLibraryGame.objects.filter(user=user).delete()
                UserLibraryGame.objects.bulk_create(user_library_games)
